 ...
 */
@Service
public class BalanceService extends AbstractRemoteService {
     ...
     * @return
     */
    public Balance get(Map<String, String> params){
        Balance balance = new Balance();

        PageResponse<List<Balance>> response = list(params);
        if(response.getCode() == ResultCode.SUCCESS && response.getData().size() > 0){
            balance = response.getData().get(0);
        }
     ...
     * @return
     */
    public PageResponse<List<Balance>> list(Map<String, String> params){
        PageResponse<List<Balance>> response = new PageResponse<List<Balance>>();
        try {
            response = get(ConstUrl.BALANCE_LIST, params, new TypeToken<PageResponse<List<Balance>>>(){}.getType());
