...
<head>
    ...
    <link href="/js/lib/bootstrap-3.3.7-dist/css/bootstrap.css" rel="stylesheet" type="text/css"/>
    ...
    <link href="/js/lib/bootstrap3-editable/css/bootstrap-editable.css" rel="stylesheet"/>
    <script src="/js/lib/bootstrap3-editable/js/bootstrap-editable.min.js"></script>
    ...
    <script language="javascript" type="text/javascript"
            src="/js/bbsSectionManage.js"></script>
...
<div class="container">
    ...
    <div class="bbs_section_info">
        <h1>
            <b>${bbsSection.name }</b>今日: <span>${bbsSection.todayPostNum }</span><em>|</em>主题:
            <span>${bbsSection.topicPostNum }</span>
        </h1>
        <h1>
            版主:
    ...
    <p class="business-title recom">
        <span>版块设置</span>
    </p>
    <div class="bbs_section_hot_list set">
        <span>发帖积分:</span><input id="postPoint" value="${bbsSection.postPoint}" autocomplete="off"type="text">
        <span>回帖积分:</span><input id="replyPoint" value="${bbsSection.replyPoint}" autocomplete="off" type="text">
        <span>加精积分:</span><input
            id="essencePoint" value="${bbsSection.essencePoint}" autocomplete="off" type="text">
        <input value="保存" onclick="setBbsSectionPointRule()" type="button">
    </div>
    ...
    <div class="bbs_type_box" style="background:#fff;">
        <ul>
            <c:forEach items="${bbsTopic}" var="topic">
                <li><a href="#" class="bbsEditable" id="bbsTopic${topic.id}" data-pk="${topic.id}" >${topic.name }</a>
                    <span class="editable-clear-x" onclick="delBbsPostByTopic(${topic.id})"></span></li>
            </c:forEach>
            <li><input value="新增" onclick="addBbsTopic()" type="button"></li>
        </ul>
    </div>
    ...
    <div class="bbs_type_box">
        <ul id="typeUl">
            ...
            <c:forEach items="${bbsTopic}" var="topic">
                <li><span
                        onclick="getBbsPostByTopic(${topic.id},this)">${topic.name }</span><em>${topic.postCount }</em></li>
            </c:forEach>
        </ul>
    </div>
    ...
    <table class="bbs_table" cellpadding="0" cellspacing="0" width="100%">
        <thead>
        <tr>
            <%--<th width="120" style="text-align: center;">全选<input type="checkbox" class="post_list_checkbox"/></th>--%>
            <th width="900"><a href="javascript:bbsPostTab('all');"
                               class="postTab"><span id="allSpan" class="onthis">最新</span></a> <a
                    href="javascript:bbsPostTab('hot');" class="postTab"><span
                    id="hotSpan">热门</span></a> <a
                    href="javascript:bbsPostTab('essence');" class="postTab"><span
                    id="essenceSpan">精华</span></a> <a
                    href="javascript:bbsPostTab('hasNoAuditing');" class="postTab"><span
                    id="hasNoAuditingSpan">未审核</span></a> <a
                    href="javascript:bbsPostTab('noAuditing');" class="postTab"><span
                    id="noAuditingSpan">未通过审核</span></a></th>
    ...
    <%--<div class="bbs_batch_btn_div">--%>
        <%--<input type="button" value="全选"/>--%>
        <%--<input type="button" value="批量通过审核"/>--%>
        <%--<input type="button" value="批量不通过审核"/>--%>
    ...
    <div class="bbs_quick_postbox" id="postbox">
        <h1>
            <img src="/images/quickpost_icon.gif"/>快速发帖
        </h1>
        <div class="bbs_quick_titlebox">

            <div class="bbs_quick_typeselect">
                <input id="bbs_type_text" type="text" placeholder="选择主题分类"
                       disabled="disabled" value="" AUTOCOMPLETE="off"/> <img src="/images/open_icon.jpg"
                                                                              height="30"/> <select id="bbs_type_select"
                                                                                                    AUTOCOMPLETE="off">
            ...
            <div class="bbs_quick_title">
                <input id="postTitle" type="text" maxlength="80" AUTOCOMPLETE="off" placeholder="请输入标题"/>
            </div>
...
<script language="javascript" type="text/javascript">
    var theTab1_box = new Array('all_essence', 'bbs_info', 'bbs_product',
        'bbs_dyeing', 'bbs_life', 'bss_other');
